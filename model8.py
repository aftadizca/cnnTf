...
def model():
    ...
    # Block 1
    model.add(tf.keras.layers.Conv2D(64, (3, 3), padding='same', activation=ACTIVATION_CONV, input_shape=IMG_SHAPE))
    ...
    model.add(tf.keras.layers.Conv2D(256, (3, 3), padding='same', activation=ACTIVATION_CONV))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Conv2D(256, (1, 1), padding='same', activation=ACTIVATION_CONV))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Dropout(0.3))
    model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2),strides=(2,2)))
    ...
    model.add(tf.keras.layers.Conv2D(512, (3, 3), padding='same', activation=ACTIVATION_CONV))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Conv2D(512, (1, 1), padding='same', activation=ACTIVATION_CONV))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Dropout(0.3))
    model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2),strides=(2,2)))
